...
int g_Mode = MODE_CREATOR_LOGO;					// 起動時の画面を設定
...
void Update(void)
{
	...
	switch (g_Mode)
	{
	case MODE_CREATOR_LOGO:		// 制作者画面の更新
		UpdateLogoCreator();
		break;
...
void Draw(void)
{
	...
	switch (g_Mode)
	{
	case MODE_CREATOR_LOGO:		// 制作者画面の描画
		SetViewPort(TYPE_FULL_SCREEN);
...
void SetMode(int mode)
{
	...
	switch (g_Mode)
	{
	case MODE_CREATOR_LOGO:		// 制作者画面の初期化
		InitLogoCreator();
		break;
...
// ベクトルの距離
//=============================================================================
float VectorDistance(XMFLOAT3 pos01, XMFLOAT3 pos02)
{

	float dis_x = pos01.x - pos02.x;
	dis_x *= dis_x;

	float dis_z = pos01.z - pos02.z;
	dis_z *= dis_z;

	float ans = sqrtf(dis_x + dis_z);
