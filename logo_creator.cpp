...
XMFLOAT4						g_LogoColor;					// フェードのカラー（α値）
...
HRESULT InitLogoCreator(void)
{
	...
	g_logo_creator.pos   = XMFLOAT3(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2.5, 0.0f);
	g_logo_creator.texNo = 1;

	g_LogoColor = { 1.0, 1.0, 1.0, 1.0 };

	// BGM再生
	PlaySound(SOUND_LABEL_BGM_creator);
...
void UpdateLogoCreator(void)
{
	...
	if (timeCnt == 65)
	{
		StopSound(SOUND_LABEL_BGM_creator);
	}
	if (timeCnt == 225)
	{
		timeCnt = 0;
